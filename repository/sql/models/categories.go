...
func (categoryL) LoadProducts(e boil.Executor, singular bool, maybeCategory interface{}) error {
	...
	for results.Next() {
		...
		err = results.Scan(&one.ProductID, &one.Name, &one.GTIN, &one.Price, &one.AddedAt, &one.UpdatedAt, &one.DeletedAt, &one.IsVisible, &one.Quantity, &one.QuantityUnit, &localJoinCol)
